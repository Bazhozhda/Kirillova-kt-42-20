...
namespace WebApplication1.Database.Configurations
{
    public class StudentsConfiguration : IEntityTypeConfiguration<Students>
    {
        ...
        public void Configure(EntityTypeBuilder<Students> builder)
        {
            ...
            builder
                .HasKey(p => p.StudentsId)
                .HasName($"pk_{TableName}_student_id");

            //Для целочисленного первичного ключа задаем автогенерацию (к каждой новой записи будет добавлять +1)
            builder.Property(p => p.StudentsId)
                    .ValueGeneratedOnAdd();
            ...
            builder.ToTable(TableName)
                .HasIndex(p => p.GroupsId, $"idx_{TableName}_fk_f_group_id");

            builder.Property(p => p.StudentIsExist)
                .IsRequired()
                .HasColumnName("c_student_is_exist")
